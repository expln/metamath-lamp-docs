...
A macro is a sequence of actions that automate some repetitive interactions with the user interface
or perform arbitrary computations.
For example, you can implement a macro that will analyze the current editor state
...
Also, mm-lamp has some predefined JavaScript functions that allow interacting with the mm-lamp internal state.
...
This document will guide you through the process of creating a simple macro that will be invoked by clicking
...
Many contemporary web browsers provide access to a console where you can run JavaScript code.
Usually you can open such a console on any tab using a menu.
For example, this is how you can open it in Firefox:

![](open_browser_console.png)

You can run arbitrary JavaScript code in this console.

The screenshot below shows a simple expression, variable definitions, function definition, and function invocation.
...
But for the editor we need to write `api.editor().apiFunction(arguments)`.
The reason is that `api.editor` is a function that can accept an editor id.
...
It is needed because the function uses the `await` keyword inside.
```js
...
We can "register" this function in the mm-lamp UI so that we can call it by clicking a button in the UI.
...
in the `macros` input parameter of `api.macro.registerMacroModule()`.

![](run_macro_button.png)

When you click "Run a macro", a small modal window opens.
In this window you can select a macro module and then select a specific macro to run.

![](list_of_macros.png)

Now you can just click "Show number of steps" to run the macro.
...
You will need to open the console and run the code below to add the macro to the UI again.
...
Apart from being inconvenient, this approach has one significant disadvantage.
When you run code like `async function showNumberOfSteps() { ... }` in the browser console,
it makes `showNumberOfSteps` a global function.
In other words, it places this function in the global namespace.
This way, you can accidentally override an existing global function with the same name,
...
4. Paste your code with macros into the `Script` text area
...
5. Type exactly the same module name in the `Module name` field as the one passed in the `moduleName`
input parameter of `api.macro.registerMacroModule`. For this example it should be "My macros".
If the value of the `moduleName` attribute and the value in the `Module name` text field do not match,
nothing critical will happen, but you may be confused by mm-lamp's behavior.
6. Make sure the `Active` checkbox is selected.
7. Click the `Save changes` button.
8. Click the small `-` button to the right of the module name dropdown to hide the additional UI elements.
...
That is how macros survive page reloads.
Moreover, mm-lamp wraps the provided code in an anonymous function before executing it, so as not to create
global functions (all the functions defined in the provided script become local functions
of the anonymous function wrapper, so they don't pollute the global namespace).
...
3. Click the small `+` button to the right of the dropdown with names of macro modules.
4. Update the script in the `Script` text area.
5. Make sure the value in the `Module name` field still matches the value passed in the `moduleName`
   input parameter of `api.macro.registerMacroModule`.
6. Click the `Save changes` button.
7. Click the small `-` button to the right of the module name dropdown to hide the additional UI elements.
